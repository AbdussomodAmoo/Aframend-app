...
# Import the predictor class from your training script
try:
    from tox21_afrodb_pipeline import AfrcanPhytochemicalToxicityPredictor
except ImportError:
    st.error("❌ Could not import the predictor class from tox21_afrodb_pipeline.py")
    st.info("Please ensure the training script is in the same directory as this Streamlit app")
    st.stop()

class StreamlitToxicityPredictor(AfrcanPhytochemicalToxicityPredictor):
    ...
    def load_models_for_streamlit(self, model_path="models/african_phytochemical_toxicity_models.pkl"):
        ...
        try:
            if not os.path.exists(model_path):
                ...
                st.info("Please run the setup script to copy the model file to the correct directory.")
                return False
            
            # Use the parent class load_models method
            super().load_models(model_path)
            self.is_loaded = True
            return True
...
def about_page():
    ...
    with tab2:
        ...
        if predictor.is_loaded and hasattr(predictor, 'training_stats'):
            st.subheader("Training Statistics")
            stats_data = []
            for endpoint, stats in predictor.training_stats.items():
                stats_data.append({
                    'Endpoint': ENDPOINT_NAMES.get(endpoint, endpoint),
                    'Training Samples': stats['n_samples'],
                    'Positive Cases': stats['n_positive'],
                    'Negative Cases': stats['n_negative'],
                    'Positive Rate': f"{stats['positive_rate']:.2%}"
                })
            
            stats_df = pd.DataFrame(stats_data)
            st.dataframe(stats_df, use_container_width=True)
        else:
            st.info("Model statistics not available. Please ensure models are loaded correctly.")
...
def toxicity_analysis_page():
    ...
    if not predictor.is_loaded:
        st.error("❌ Models not loaded. Please ensure the model file is available.")
        st.info("Run the download script to get the required model files.")
        return
